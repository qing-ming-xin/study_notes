...
**定义位于类声明中的函数都自动成为内联函数，也可以在类声明之外定义成员函数，并使其成为内联函数。**
...
类中使用关键字static，该常量将与其他静态变量存储在一起，而不是存储在对象中，该变量被所有类对象共享。



声明枚举时，使用class或static关键字，使用枚举名来限定枚举量，不同枚举定义中的枚举量就不会发生名称冲突。  
...
```c++
className operator+(const className &t) const;
```



**在函数后加const的意义：**对不改变数据成员的函数加上const关键词进行标识，可提高程序的可读性。限制函数修改数据成员的值。
...
- 编写函数定义。因为其不是成员函数，所以不使用::限定符。在定义中不使用关键字friend.
...
在C++中，接受一个参数的构造函数为将类型与该参数相同的值转换为类提供了蓝图。只有接受一个参数的构造函数才能作为转换函数。C++新增关键字`explicit`用于关闭这种自动特性。`explicit className(int perm)`,但仍然允许显式转换。



构造函数（一个参数）：可用于从某种类型到类类型的转换

转换函数：用于类类型到某种类型的转换（用户定义的强制类型转换）
...
```c++
operator int() const;//const可根据实际情况而定
```
...
不要在类声明中初始化静态成员变量，这是因为声明描述了如何分配内存，但并不分配内存。



**静态数据成员在类声明中声明，在包含类方法的文件中初始化。初始化时使用作用域运算符来指出静态成员所属的类。但如果静态成员时整型或枚举型const，则可以在类声明中初始化。**
